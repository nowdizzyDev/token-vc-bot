...
async function reconnectAll(client) {
    ...
    for (const row of normalTokens) {
        try {
            ...
            await delay(2000);
        } catch (e) { console.error(`❌ Normal token hatası:`, e.message); }
    }
    ...
    for (const row of streamerTokens) {
        try {
            ...
            await delay(2000);
        } catch (e) { console.error(`❌ Streamer token hatası:`, e.message); }
    }
    ...
    for (const row of presenceRows) {
        try {
            ...
            await delay(2000);
        } catch (e) { console.error(`❌ Presence hatası:`, e.message); }
    }
}
